...
def run(path):
    ...
    with sync_playwright() as p:
        ...
        # 4 sim control -> live panel + worked run
        pg.evaluate("var n=document.getElementById('N-select'); n.value='2'; n.dispatchEvent(new Event('change',{bubbles:true}))")
        pg.evaluate("var v=document.getElementById('v0-slider'); v.value='1.55'; v.dispatchEvent(new Event('input',{bubbles:true}))")
        pi = pg.evaluate("document.getElementById('phys-pi').textContent")
        worked = pg.evaluate("document.getElementById('phys-worked').textContent")
        check('4a. live count from current N (314)', '314' in pi)
        check('4b. digit phrase (first 3 digits of π)', 'first 3 digits' in pi)
        check('4c. worked run uses current values', 'N = 2' in worked and 'your settings' in worked and '1.55' in worked)
        check('4d. worked Π recomputed (314)', '314' in worked)

        # 5 mini slider -> sim control
        pg.evaluate("var m=document.getElementById('phys-n'); m.value='1'; m.dispatchEvent(new Event('input',{bubbles:true}))")
        check('5a. mini slider drives N-select',
              pg.evaluate("document.getElementById('N-select').value") == '1')
        check('5b. count flips to 31',
              '31' in pg.evaluate("document.getElementById('phys-pi').textContent"))
        ...
        check('6a. replay sends live N (1)', cap and cap['N'] == 1)
        check('6b. replay sends live v0 (1.55)', cap and abs(cap['v0'] - 1.55) < 1e-9)
        ...
        if sheet:
            ...
            check('8c. snapshot carries CURRENT values', 'N = 1' in sheet and '1.55' in sheet)
